...
function warningSeverity(kind, fallback) {
    if (kind in AST_SEVERITY)
        return AST_SEVERITY[kind];
...
function analyseSource(text, file, analyser, tsParser) {
...
function decodeFragments(text) {
    const out = [];
    const push = (decoded, index) => {
...
function findingsForDecoded(fragments, file) {
    const out = [];
...
export function scanPlugin(files) {
    ...
    const hasSecrets = findings.some(f => f.rule === 'exfil-secrets'
        || f.rule === 'exfil-keys'
        ...
        || f.rule === 'env-read');
    const hasNetwork = findings.some(f => f.rule === 'network-egress'
    ...
    for (const f of findings)
        dedup.set(`${f.rule}:${f.file}:${f.line}:${f.hint}`, f);
